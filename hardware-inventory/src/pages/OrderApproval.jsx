...
export default function OrderApproval() {
    ...
    async function handleAddNote(order, note) {
        if (!note) return;
        const { error } = await supabase.from('purchase_orders').update({ notes: note }).eq('id', order.id);
        if (!error) toast.success('Note updated');
    }

    function getStockBar(product) {
        if (!product) return null;
        const pct = Math.min(100, Math.round((product.current_stock / Math.max(product.reorder_point * 3, 1)) * 100));
        const color =
                                ...
                                <div className="flex flex-col lg:flex-row lg:items-center gap-4">
                                    <div className="flex-1">
                                        ...
                                        <p className="text-xs text-gray-500 mb-3">
                                            {order.triggered_by === 'ai_prediction' ? 'Forecast Recommendation' : 'Manual'} | SKU: {order.products?.sku} | Supplier: {order.products?.supplier_name || 'N/A'}
                                        </p>
                                        ...
                                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-3">
                                            ...
                                            <div className="bg-gray-50 rounded-lg p-2">
                                                ...
                                                <p
                                                    className={`font-bold ${
                                                        order.products?.current_stock <= order.products?.reorder_point / 2
                                                            ? 'text-red-600'
                                                            : order.products?.current_stock <= order.products?.reorder_point
                                                                ? 'text-yellow-600'
                                                                : 'text-green-700'
                                                    }`}
                                                >
